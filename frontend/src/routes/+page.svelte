<script lang="ts">
	...
	function handleUpdate(updatedPaths: WatchedPath[]) {
		...
		const enabledDirs = updatedPaths.filter((p) => p.enabled).map((p) => p.directory);
	...
	onMount(async () => {
		...
		try {
			...
			if (backendStatusResp) {
				...
				// Use cached/stored paths when backend is offline
	...
	async function startWatching() {
		...
		try {
			// Get enabled paths
			const enabledPaths = paths.filter((p) => p.enabled).map((p) => p.directory);
	...
	let disableStop = $derived(
		isStopping || watchStatus !== 'watching' || backendStatus !== 'online'
	);
</script>
